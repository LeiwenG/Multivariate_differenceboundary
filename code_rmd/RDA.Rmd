...
Figure 11: Maps of county-level covariates: adult cigarette smoking rates (left), percentage of residents older than 65 years old (middle) and unemployed residents (right).
```{r, fig.height=5, fig.width=10}
ca.poly$old = rate_lung1$old
...
brks_fit_old = quantile(ca.poly$old, c(0, 0.2, 0.4, 0.6, 0.8, 1))
class.old = classIntervals(var=ca.poly$old, n=5, style="fixed",
                           fixedBreaks=brks_fit_old, dataPrecision=4)
color.code.old = findColours(class.old, color.pallete)
...
plot(ca.poly, col = color.code.old)
leg.txt = c("9.07 - 12.05", "12.05 - 13.21", "13.21 - 15.63", "15.63 - 20.66", "20.66 - 26.07")
legend("bottomleft", title="Old (%)", legend=leg.txt, xpd = TRUE, cex=1.25, bty="n", horiz = FALSE,
       fill = color.pallete)
...
####### b. Table 3: Names of adjacent counties that have significant boundary effect for each cancer when $\delta = 0.025$ 
```{r}
...
Figure 7 (a): Difference boundaries (highlighted in red) detected by MAGAR in SIR map for four cancers individually when $\delta = 0.025$.
```{r, fig.height=7, fig.width=10}
...
Figure 8: Shared difference boundaries (highlighted in red) detected by MDAGAR for each pair of cancers in SIR map when $\delta = 0.025$.
```{r, fig.height=7, fig.width=10}
alpha_nc = 0.025
...
Figure 9: Mutual cross-cancer difference boundaries (highlighted in red) detected by MDAGAR for each pair of cancers in SIR map when $\delta = 0.025$.
```{r, fig.height=7, fig.width=10}
alpha_n1 = 0.025
...
####### b. Names of adjacent counties that have significant boundary effects from the MCAR model for each cancer when $\delta = 0.025$
```{r}
# Set threshold for FDR: 0.025
alpha_n = 0.025
T1 = sum(FDR_est1<=alpha_n)
...
Figure 7 (b): Difference boundaries (highlighted in red) detected by MCAR in SIR map for four cancers individually when $\delta = 0.025$.
```{r, fig.height=7, fig.width=10}
# Difference boundary for each cancer with FDR <= 0.025
est_diff1 <- as.numeric(pvij1 >= threshold1[T1])
...
ARDP_joint_diff_smoke<-function(y=Y, x1=as.matrix(X1[,1:2]), x2=as.matrix(X2[,1:2]), x3 = as.matrix(X3[,1:2]), x4 = as.matrix(X4[,1:2]), 
  ...
  for(iter in 1:runs){
    ...
    pro_beta1=rmvnorm(1,beta1,0.000001*diag(ncol(x1)))
    ...
    pro_beta2=rmvnorm(1,beta2,0.000005*diag(ncol(x2)))
    ...
    pro_beta3=rmvnorm(1,beta3,0.00001*diag(ncol(x3)))
    ...
    pro_beta4=rmvnorm(1,beta4,0.0000005*diag(ncol(x4)))
...
#### 4.2 Account for smoking and age
```{r}
# Construct covariates matrix
X = as.matrix(bdiag(bdiag(X1[,c(1,2,4)], X2[,c(1,2,4)]), bdiag(X3[,c(1,2,4)],X4[,c(1,2,4)])))

ARDP_joint_diff<-function(y=Y, x1=as.matrix(X1[,c(1,2,4)]), x2=as.matrix(X2[,c(1,2,4)]), x3 = as.matrix(X3[,c(1,2,4)]), x4 = as.matrix(X4[,c(1,2,4)]), 
                          X = X, E = E, Minc, alpha=1, q = 4, n.atoms=15, runs=10000, burn=1000){
  #y:       data
  #x:       covariates
  #n.atoms: number of atoms in the mixture dist.
  ...
  for(iter in 1:runs){
    ...
    pro_beta1=rmvnorm(1,beta1,0.0000005*diag(ncol(x1)))
    MHrate1=sum(-E1*exp(pro_beta1%*%t(x1)+theta[u][1:n])+y1*(pro_beta1%*%t(x1)+theta[u][1:n]))-
      sum(-E1*exp(beta1%*%t(x1)+theta[u][1:n])+y1*(beta1%*%t(x1)+theta[u][1:n]))  
    ...
    pro_beta3=rmvnorm(1,beta3,0.000006*diag(ncol(x3)))
    MHrate3=sum(-E3*exp(pro_beta3%*%t(x3)+theta[u][(2*n+1):(3*n)])+y3*(pro_beta3%*%t(x3)+theta[u][(2*n+1):(3*n)]))-
      sum(-E3*exp(beta3%*%t(x3)+theta[u][(2*n+1):(3*n)])+y3*(beta3%*%t(x3)+theta[u][(2*n+1):(3*n)]))  
    ...
    pro_beta4=rmvnorm(1,beta4,0.0000003*diag(ncol(x4)))
    MHrate4=sum(-E4*exp(pro_beta4%*%t(x4)+theta[u][(3*n+1):(4*n)])+y4*(pro_beta4%*%t(x4)+theta[u][(3*n+1):(4*n)]))-
      sum(-E4*exp(beta4%*%t(x4)+theta[u][(3*n+1):(4*n)])+y4*(beta4%*%t(x4)+theta[u][(3*n+1):(4*n)]))  
...
set.seed(123)
mcmc_samples=ARDP_joint_diff(y=Y, x1=as.matrix(X1[,c(1,2,4)]), x2=as.matrix(X2[,c(1,2,4)]), x3 = as.matrix(X3[,c(1,2,4)]), x4 = as.matrix(X4[,c(1,2,4)]), 
                             X = X, E=E, Minc, alpha=1, q = 4, n.atoms=15, runs=30000, burn=20000)
...
#### 4.3 Account for smoking, age and unemployed
```{r}
# Construct covariates matrix
X = as.matrix(bdiag(bdiag(X1[,c(1,2,4,7)], X2[,c(1,2,4,7)]), bdiag(X3[,c(1,2,4,7)],X4[,c(1,2,4,7)])))

ARDP_joint_diff<-function(y=Y, x1=as.matrix(X1[,c(1,2,4,7)]), x2=as.matrix(X2[,c(1,2,4,7)]), x3 = as.matrix(X3[,c(1,2,4,7)]), x4 = as.matrix(X4[,c(1,2,4,7)]), 
                          X = X, E = E, Minc, alpha=1, q = 4, n.atoms=15, runs=10000, burn=1000){
  #y:       data
  #x:       covariates
  #n.atoms: number of atoms in the mixture dist.
...
set.seed(123)
mcmc_samples=ARDP_joint_diff(y=Y, x1=as.matrix(X1[,c(1,2,4,7)]), x2=as.matrix(X2[,c(1,2,4,7)]), x3 = as.matrix(X3[,c(1,2,4,7)]), x4 = as.matrix(X4[,c(1,2,4,7)]), 
                             X = X, E=E, Minc, alpha=1, q = 4, n.atoms=15, runs=30000, burn=20000)

sample.mcmc = cbind(mcmc_samples$taus, mcmc_samples$rho1, mcmc_samples$rho2, mcmc_samples$rho3, mcmc_samples$rho4)
```

##### 4.3.1 Coefficients (covariates slopes) estimates in Table 5
```{r}
#Summary function
mysummary = function(invector) {
  c(mean(invector), quantile(invector, .025), quantile(invector,.975))
...
cov_effect = round(t(apply(mcmc_samples$beta, 2, mysummary)),3)
beta_est = paste(cov_effect[,1], " (", cov_effect[,2], ", ", cov_effect[,3], ")", sep="")
beta_table = data.frame(t(matrix(beta_est, ncol = 4, byrow = TRUE)))
colnames(beta_table) = c("Lung", "Esophageal", "Larynx", "Colorectal")
rownames(beta_table) = c("Intercept", "Smoking", "Age", "Unemployed")
kable(beta_table)
...
##### 4.3.2 Difference boundaries for each cancer individually

Figure 10 (c): Difference boundaries (highlighted in red) detected by MDAGAR after accounting for smoking, age and unemployed for four cancers
```{r, fig.height=7, fig.width=10}
phis <- mcmc_samples$phi
phis1 <- phis[,1:58][,order(final_perm)]
phis2 <- phis[,59:116][,order(final_perm)]
